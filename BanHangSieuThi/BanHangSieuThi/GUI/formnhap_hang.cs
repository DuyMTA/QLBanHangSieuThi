...
namespace BanHangSieuThi.GUI
{
    public partial class formnhap_hang : Form
    {
        ...
        private void gunaBtLuu_Click(object sender, EventArgs e)
        {
            if (dem == 1)
            {
                using (connection = new SqlConnection(connectStr.getConnectionString(fromdangnhap.checkConnectionString)))
                {
                    connection.Open();
                    cmd = connection.CreateCommand();
                    cmd.CommandText = "INSERT INTO dbo.tblSanPham VALUES  ( '" + guTbMaSP.Text + "' , N'" + guTbTenSP.Text + "' , N'LH3' , '" + guTbSLSP.Text + "' , '" + guTbLoiNhuanSP.Text + "' , '" + guTbGiaNhapSP.Text + "' , '" + guTbGiaBanSP.Text + "' , N'" + guTbMoTa.Text + "' , N'" + guTbNXS.Text + "' , N'' , N'LAZADA' )";
                    try
                    {
                        cmd.ExecuteNonQuery();
            ...
            if (dem == 2)
            {
                ...
                using (connection = new SqlConnection(connectStr.getConnectionString(fromdangnhap.checkConnectionString)))
                {
                    connection.Open();
                    cmd = connection.CreateCommand();
                    cmd.CommandText = "UPDATE dbo.tblSanPham SET  MaSP='" + guTbMaSP.Text + "' ,TenSP=N'" + guTbTenSP.Text + "' , MaLH=N'LH3',  SoLuong='" + guTbSLSP.Text + "' , LoiNhuan='" + guTbLoiNhuanSP.Text + "' , GiaNhap='" + guTbGiaNhapSP.Text + "' , GiaBan='" + guTbGiaBanSP.Text + "' ,MoTa= N'" + guTbMoTa.Text + "' , NSX=N'" + guTbNXS.Text + "' ,HinhAnh=N'' , NhaCC=N'LAZADA' WHERE MaSP='" + masp + "'";
                    try
                    {
                        cmd.ExecuteNonQuery();
        ...
        private void guTbLoaiSP_TextChanged(object sender, EventArgs e)
        {
